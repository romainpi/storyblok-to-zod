...
function analyzeGeneratedContent(content: string): { imports: Set<string>; schemas: string[] } {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // Detect schema start - any const/export const that uses Zod or iSbStoryDataSchema
    if ((trimmedLine.startsWith("export const ") || trimmedLine.startsWith("const ")) && 
        (trimmedLine.includes(" = z.") || trimmedLine.includes("= z.") || trimmedLine.includes(" = iSbStoryDataSchema"))) {
      inSchema = true;
      currentSchema = line;
...
export function formatFinalOutput(
  ...
  try {
    ...
    // Fix any remaining PLACEHOLDER_IMPORT_PATH imports
    const fixedImports = finalImports.map(importStatement => {
      if (importStatement.includes('PLACEHOLDER_IMPORT_PATH')) {
        // Replace with the correct import for StoryblokRichtext (and any other types from storyblok.d)
        return importStatement.replace('PLACEHOLDER_IMPORT_PATH', '~/types/storyblok.d');
      }
      return importStatement;
    });

    // Note: We don't re-export external types like ISbStoryData or StoryblokRichtext
    // Users should import them directly from their respective packages:
    // - ISbStoryData, ISbMultipleStoriesData from 'storyblok-js-client'
    // - StoryblokRichtext from '~/types/storyblok.d'
    ...
    Tracer.log(
      LogLevel.DEBUG,
      `Formatted output with ${fixedImports.length} imports and ${organizedSchemas.length} schemas`,
      "formatFinalOutput"
    );
